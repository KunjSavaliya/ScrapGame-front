...
import NewLunch from '../Tablet/NewLunch/page';
...
function Tablet() {
  ...
  const clearSearch = () => {
    setSearchQuery(''); // Clear the search query when closing the search
  ...
  const filteredGames = TabletGame?.data?.map(section => ({
    ...section,
    games: section.games.filter(game => 
      game.title.toLowerCase().includes(searchQuery.toLowerCase()) && // Filter based on the search query
  ...
  return (
    <div className="flex flex-col items-center justify-center p-5 ">
      ...
      <div className="bg-[#69a2ff] p-2 grid grid-cols-3 gap-6 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 rounded-lg mb-3 border-white" style={{ boxShadow: "rgb(157 194 255) 0px 2px 4px, rgb(157 194 255) 0px 7px 13px -3px, rgb(157 194 255) 0px -2px 0px inset" }}>
        <h1 className="pl-5 text-2xl text-white font-lighter">Tablet</h1>
      </div>
      
      <NewLunch filteredGames={searchQuery} /> {/* Pass the filtered games to NewLunch */}
